...
// flash bank ht32f5xxx <base> <size> 0 0 <target#>
FLASH_BANK_COMMAND_HANDLER(ht32f5xxx_flash_bank_command)
{
    if (CMD_ARGC < 6)
...
static int ht32f5xxx_get_flash_status(struct flash_bank *bank, uint32_t *status)
{
    struct target *target = bank->target;
    return target_read_u32(target, FMC_REG_BASE + FMC_REG_OPCR, status);
}

static int ht32f5xxx_wait_status_busy(struct flash_bank *bank, int timeout)
{
    uint32_t status;
    int retval = ERROR_OK;
    ...
    for (;;) {
        retval = ht32f5xxx_get_flash_status(bank, &status);
        if (retval != ERROR_OK)
            return retval;
...
static int ht32f5xxx_erase(struct flash_bank *bank, int first, int last)
{
    ...
    LOG_DEBUG("ht32f5xxx erase: %d - %d", first, last);
    ...
    for(int i = first ; i <= last; ++i){
        ...
        // wait
        retval = ht32f5xxx_wait_status_busy(bank, FLASH_ERASE_TIMEOUT);
        if (retval != ERROR_OK)
            return retval;

        LOG_DEBUG("ht32f5xxx erased page %d", i);
...
static int ht32f5xxx_write(struct flash_bank *bank, const uint8_t *buffer,
                           uint32_t offset, uint32_t count)
{
    struct target *target = bank->target;

    LOG_DEBUG("ht32f5xxx flash write: 0x%x 0x%x", offset, count);
    ...
    for(uint32_t i = 0; i < count; i += 4){
        ...
        LOG_DEBUG("ht32f5xxx flash write word 0x%x 0x%x 0x%08x", i, addr, word);
    ...
    LOG_DEBUG("ht32f5xxx flash write success");
    return ERROR_OK;
}

static int ht32f5xxx_probe(struct flash_bank *bank)
{
    int page_size = 1024;
    int num_pages = bank->size / page_size;

    LOG_INFO("ht32f5xxx probe: %d pages, 0x%x bytes, 0x%x total", num_pages, page_size, bank->size);
...
static int ht32f5xxx_auto_probe(struct flash_bank *bank)
{
    return ht32f5xxx_probe(bank);
}

static int ht32f5xxx_protect_check(struct flash_bank *bank)
{
    struct target *target = bank->target;
    uint32_t ob_pp[4];
    uint32_t ob_cp;
    ...
    LOG_INFO("ht32f5xxx opt byte: %04x %04x %04x %04x %04x", ob_pp[0], ob_pp[1], ob_pp[2], ob_pp[3], ob_cp);
...
static int ht32f5xxx_info(struct flash_bank *bank, char *buf, int buf_size)
{
    const char *info = "ht32f5xxx flash";
    strncpy(buf, info, MIN((int)strlen(info), buf_size));
    return ERROR_OK;
}

static int ht32f5xxx_mass_erase(struct flash_bank *bank)
{
    struct target *target = bank->target;
...
COMMAND_HANDLER(ht32f5xxx_handle_mass_erase_command)
{
    if (CMD_ARGC < 1)
        return ERROR_COMMAND_SYNTAX_ERROR;
...
COMMAND_HANDLER(ht32f5xxx_handle_test_write)
{
    if (CMD_ARGC < 1)
        return ERROR_COMMAND_SYNTAX_ERROR;
    ...
    retval = ht32f5xxx_erase(bank, 0, 0);
    if (retval != ERROR_OK)
        return retval;
...
static const struct command_registration ht32f5xxx_exec_command_handlers[] = {
    {
        .name = "mass_erase",
        .handler = ht32f5xxx_handle_mass_erase_command,
        .mode = COMMAND_EXEC,
        .usage = "bank_id",
    ...
    {
        .name = "test_write",
        .handler = ht32f5xxx_handle_test_write,
        .mode = COMMAND_EXEC,
        .usage = "bank_id",
...
static const struct command_registration ht32f5xxx_command_handlers[] = {
    {
        .name = "ht32f5xxx",
        .mode = COMMAND_ANY,
        .help = "ht32f5xxx flash command group",
        .usage = "",
        .chain = ht32f5xxx_exec_command_handlers,
    },
    COMMAND_REGISTRATION_DONE
};

struct flash_driver ht32f5xxx_flash = {
    .name = "ht32f5xxx",
    .commands = ht32f5xxx_command_handlers,
    .flash_bank_command = ht32f5xxx_flash_bank_command,

    .erase          = ht32f5xxx_erase,
    .protect        = ht32f5xxx_protect,
    .write          = ht32f5xxx_write,
    .read           = default_flash_read,
    .probe          = ht32f5xxx_probe,
    .auto_probe     = ht32f5xxx_auto_probe,
    .erase_check    = default_flash_blank_check,
    .protect_check  = ht32f5xxx_protect_check,
    .info           = ht32f5xxx_info,
};
